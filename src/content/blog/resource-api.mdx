...
export class MeuComponente {
	...
	atualizarTodo() {
		this.recursoTodo.value.update((valor) => {
...
Podemos atualizar os dados localmente usando o método `update` do signal `value`.
...
export class MeuComponente {
	...
	atualizar() {
		this.recursoTodos.refresh()
	}
}
```

A função `refresh` vai rodar a função `loader` novamente. Se você chamar a função `refresh` várias vezes, a função `loader` será chamada apenas uma vez até que a requisição anterior seja finalizada (comportamento semelhante ao `exhaustMap` no RxJS).
...
recursoTodos = resource({
	request: () => ({ limite: this.limite(), consulta: this.consulta() }),
